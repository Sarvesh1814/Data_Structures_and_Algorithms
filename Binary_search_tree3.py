class BinarySearchTreeNode:
    ...
    def delete_node_2(self,val):
        if val < self.data:
            if self.left:
                self.left= self.left.delete_node_2(val)
        if val > self.data:    
            if self.right:
                self.right=self.right.delete_node_2(val)
        else:
            if self.left is None and self.right is None:
                return None
            if self.left is None:
            ...
            self.data = max_val
            self.left=self.left.delete_node_2(max_val)
